...
class hardwaresequenceProcessor:
    ...
    def __init__(
        self,
        csv_path: str = "hardware_activity.csv",
        mongo_uri: str = "mongodb://localhost:27017/",
        db_name: str = "hardware_sequences",
    ):
        """
        ...
        self.sequences_collection = self.db.sequences
    ...
    def _ensure_indexes(self):
        ...
        self.sequences_collection.create_index([("sequence_id", ASCENDING)], unique=True)
        # Index on timestamps for range queries
        self.sequences_collection.create_index([("start_time", ASCENDING)])
        self.sequences_collection.create_index([("end_time", ASCENDING)])
        # Index on label for filtering
        self.sequences_collection.create_index([("label", ASCENDING)])
        # Index on config_key for metadata queries
        self.metadata_collection.create_index([("config_key", ASCENDING)], unique=True)
        self.config_collection.create_index([("config_key", ASCENDING)], unique=True)

    def _get_config_key(self) -> str:
        """Generate config key based on current configuration."""
        return f"{self.window_size}_{self.sequence_gap_threshold}_{self.min_sequence_length}"
    ...
    def _identify_sequences(self) -> List[Dict]:
        ...
        """
        # Load existing labels from MongoDB
        existing_labels = {}
        config_key = self._get_config_key()
    ...
    def _save_to_mongodb(self, config_key: str) -> Dict:
        """Save complete processor state to MongoDB."""
        try:
            # Save metadata
            ...
            self.sequences_collection.delete_many({"config_key": config_key})

            # Save sequences in bulk for better performance
            if self.sequences:
                sequence_docs = []
                for seq in self.sequences:
                    doc = {
    ...
    def _load_from_mongodb(self, config_key: str) -> Dict:
        ...
        try:
            ...
            self.hardware_names = metadata_doc.get("hardware_names", [])
    ...
    def clear_config_data(self, config_key: Optional[str] = None) -> Dict:
        """
        Clear all data for a specific configuration or current configuration.
        Useful for resetting or cleaning up old data.

        Args:
            config_key: Configuration key to clear (if None, uses current config)
        ...
        try:
            # Delete sequences
            seq_result = self.sequences_collection.delete_many({"config_key": config_key})

            # Delete metadata
            meta_result = self.metadata_collection.delete_one({"config_key": config_key})
    ...
    def list_available_configs(self) -> Dict:
        """
        List all available configurations stored in MongoDB.
    ...
    def export_labeled_sequences(self, label_filter: Optional[List[str]] = None) -> Dict:
        """
        Export labeled sequences for machine learning or analysis.

        Args:
            label_filter: List of labels to export (if None, exports all labeled sequences)

        Returns:
            Dictionary with exported sequences
        """
        try:
            config_key = self._get_config_key()
