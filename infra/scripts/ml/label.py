...
df = pd.read_csv("hardware_activity.csv", parse_dates=["timestamp"])
df = df.sort_values("timestamp")

print(f"Loaded {len(df)} hardware events from {df.timestamp.min()} to {df.timestamp.max()}")

# --- PIVOT DATA TO MULTIVARIATE FORMAT ---
pivoted = df.pivot_table(index="timestamp", columns="hardware_name", values="state", aggfunc="sum")
...
def identify_sequences(windowed_data, gap_threshold_seconds):
            ...
            if current_sequence_start is None:
                ...
                if gap > gap_threshold_seconds:
                    ...
                    if len(current_sequence_windows) >= MIN_SEQUENCE_LENGTH:
                        ...
                        sequences.append(
                            {
                                ...
                                "duration_minutes": (
                                    last_activity_time - current_sequence_start
                                ).total_seconds()
                                / 60,
                        ...
                        print(
                            f"Completed sequence: {current_sequence_start} to {last_activity_time} ({len(current_sequence_windows)} windows)"
                        )
    ...
    if current_sequence_start is not None and len(current_sequence_windows) >= MIN_SEQUENCE_LENGTH:
        ...
        sequences.append(
            {
                ...
                "end_time": last_activity_time,
                "windows": current_sequence_windows.copy(),
                "duration_minutes": (last_activity_time - current_sequence_start).total_seconds()
                / 60,
        ...
        print(
            f"Completed final sequence: {current_sequence_start} to {last_activity_time} ({len(current_sequence_windows)} windows)"
        )
...
for seq_idx, sequence in enumerate(sequences):
    ...
    for hardware in sequence_data.columns:
        ...
        if total_activity > 0:
            print(f"  {hardware}: {total_activity} total activations")

    label = "Ignore"  # input("\nEnter label for this ENTIRE SEQUENCE (Ignore, Log, Notify, Alarm): ").strip()

    labeled_sequences.append({**sequence, "label": label, "sequence_id": seq_idx + 1})
...
for seq in labeled_sequences:
    ...
    sequence_array = sequence_windows.values  # Shape: (n_windows, n_hardwares)

    # Add metadata as additional features (optional)
    n_windows, n_hardwares = sequence_array.shape
    ...
    # Combine hardware data with metadata
...
output_data = {
    ...
    "hardware_names": list(pivoted_windowed.columns),
    "config": {
