...
def extract_features(sequence):
    ...
    # Time-based
    start_time = datetime.fromisoformat(sequence['start_time']) if sequence.get('start_time') else None
    if start_time:
    ...
    if raw_events:
        ...
        timestamps = [datetime.fromisoformat(e['timestamp']) for e in raw_events if e.get('timestamp')]
        if len(timestamps) > 1:
        ...
        state_changes = sum(1 for i in range(len(raw_events)-1)
...
def main():
    parser = argparse.ArgumentParser(description='Predict labels for sensor sequences using saved models')
    parser.add_argument('--model', choices=['rf', 'xgb'], default='rf', help='Which model to use')
    parser.add_argument('--model-path', default='random_forest_model.pkl', help='Path to Random Forest model (joblib)')
    parser.add_argument('--xgb-path', default='xgboost_model.pkl', help='Path to XGBoost model (joblib)')
    parser.add_argument('--encoder-path', default='label_encoder.pkl', help='Path to label encoder (joblib)')
    parser.add_argument('--input', required=True, help='Path to input JSON (single sequence, list, or dataset with sequences key)')
    parser.add_argument('--sequence-id', type=int, help='Optional sequence_id to predict (if input contains many)')
    ...
    for seq in sequences:
        ...
        print(f"Sequence {out['sequence_id']}: {out['predicted_label']} (probs: {out['probabilities']})")
    ...
    if args.output_csv:
        ...
        with open(args.output_csv, 'w', newline='') as f:
            writer = csv.writer(f)
            # header
            header = ['sequence_id', 'predicted_label'] + list(results[0]['probabilities'].keys()) if results else ['sequence_id', 'predicted_label']
            writer.writerow(header)
            for r in results:
                row = [r['sequence_id'], r['predicted_label']] + [r['probabilities'].get(k, 0) for k in header[2:]]
                writer.writerow(row)
